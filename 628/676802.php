...
while($n > 0) {
    $n--;
    $null = UtilIO::getInt();
    [$null, $point] = UtilIO::getIntArray();
    $tags = UtilIO::getStringArray();
    foreach($tags as $tag) {
        if(array_key_exists($tag,$counter)) {
            $counter[$tag] += $point;
            continue;
        };
        $counter[$tag] = $point;
...
foreach($counter as $k => $v) {
    $sortable[] = ["key"=>$k,"value"=>$v];
}
array_multisort(array_column($sortable,"value"),SORT_DESC,array_column($sortable,"key"),SORT_ASC,$sortable);
$limit = 10;
foreach($sortable as $element) {
    UtilIO::echo("${element['key']} ${element['value']}");
    $limit--;
    if($limit < 1) break;
}
