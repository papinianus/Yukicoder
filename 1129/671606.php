...
$inputs = UtilIO::getIntArray();
UtilIO::echo(judge(...$inputs));

function judge(int $a, int $b, int $c, int $d):string {
    $null = "null";
    $true = "tRue";
    $draw = "Draw";
    if($a > $c) return $null;
    if($c > $a) return $true;
    if(wins($b,$d)) return $null;
    if(wins($d,$b)) return $true;
    return $draw;
}
function wins(int $a, $b):bool {
    return match("$a$b") {
        "01","12","20" => true,
        default => false,
    };
