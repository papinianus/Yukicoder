...
$a = new Triple(UtilIO::getIntArray());
$b = new Triple(UtilIO::getIntArray());
for($i = 0; $i < 3; $i++) {
    for($j = 0; $j < 3; $j++) {
        if($a->exchange($i,$b->getElementOf($j))->is_kadomatsu() && $b->exchange($j,$a->getElementOf($i))->is_kadomatsu()) {
            UtilIO::echoYNPascal(true);
            exit;
        }
    }
}
UtilIO::echoYNPascal(false);
